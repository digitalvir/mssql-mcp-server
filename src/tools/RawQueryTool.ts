...
export class RawQueryTool implements Tool {
  [key: string]: any;
  name = "raw_query";
  description =
  ...
  inputSchema = {
    ...
    properties: {
      ...
      database: {
        type: "string",
        description:
          "Optional database name. If provided, USE [db] is prepended (requires server-level access for cross-DB).",
      },
      environment: {
        type: "string",
  ...
  async run(params: any) {
    try {
      ...
      console.error(
        `raw_query${database ? ` on [${database}]` : ""}: ${finalQuery.substring(0, 200)}${
          finalQuery.length > 200 ? "..." : ""
        }`
      );
      ...
      return {
        success: true,
        message: `Executed. ${recordsets.length} recordset(s).${
          result.rowsAffected ? ` rowsAffected: ${JSON.stringify(result.rowsAffected)}` : ""
        }`,
