...
export class ReadDataTool {
    ...
    async run(params) {
        try {
            ...
            console.error(`Executing validated SELECT query${database ? ` on [${database}]` : ""}${limitAdded ? ` (auto-limited to ${maxRowsToUse} rows)` : ""}: ${limitedQuery.substring(0, 200)}${limitedQuery.length > 200 ? "..." : ""}`);
            // Execute the query
